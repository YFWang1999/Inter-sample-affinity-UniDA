...
def get_loader(source_path, target_path, evaluation_path, transforms,
               batch_size=34, return_id=False, balanced=False, val=False, val_data=None):
    ...
    if balanced:
        ...
        sampler = WeightedRandomSampler(source_weights,
                                        len(source_folder.labels),replacement=False)
        ...
        source_loader = torch.utils.data.DataLoader(
            ...
            drop_last=True,
            num_workers=4)
        source_loader1 = torch.utils.data.DataLoader(
            source_folder,
            batch_size=34,
            #sampler=sampler,
            drop_last=False,
    ...
    target_loader1 = torch.utils.data.DataLoader(
        target_folder_train,
        batch_size=34,
        shuffle=False,
...
def get_loaderclu(source_path, target_path, evaluation_path, transforms,
               batch_size=34, return_id=False, balanced=False, val=False, val_data=None):
    source_folder = ImageFolder(os.path.join(source_path),
                                transforms[source_path],
                                return_id=return_id)
    ...
    return source_loader1, target_loader1
