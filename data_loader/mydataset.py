...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, image_list, transform=None, target_transform=None, return_paths=False,
        ...
        cluster=np.array([-1 for i in range(len(labels))])
    ...
    def __getitem__(self, index):
        ...
        clusterID=self.cluster[index]
        img = self.loader(path)
        img = self.transform(img)
        if self.target_transform is not None:
            target = self.target_transform(target)
        if self.return_paths:
            return img, target, path,clusterID
        elif self.return_id:
            return img,target ,index,clusterID
        else:
            return img, target,clusterID
