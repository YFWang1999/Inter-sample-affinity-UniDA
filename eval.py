...
def test(step, dataset_test, name, n_share, G, Cs, thr,
         open_class = None, open=False, entropy=False, doubleC=True):
    G.eval()
    for c in Cs:
        c.eval()
    ...
    for batch_idx, data in enumerate(dataset_test):
        with torch.no_grad():
            ...
            if entropy:
                pred_unk = -torch.sum(out_t * torch.log(out_t), 1)
                ind_unk = np.where(entr > thr*2)[0]
            elif doubleC:
                out_open = Cs[1](feat)
                out_open = torch.softmax(out_open,dim=1)
                #pred_unk = out_open.data.max(1)[0].cpu().numpy()
                pred_unk1 = -torch.sum(out_open * torch.log(out_open+1e-5), 1).cpu().numpy()
                #pred_plus = out_open.data.max(1)[1]
                #pred_unk = pred.eq(pred_plus)
                ent1 = -torch.sum(out_open * torch.log(out_open+1e-5), 1).data.cpu().numpy()
                #ent2 = -torch.sum(out_t * torch.log(out_t+1e-5), 1).data.cpu().numpy()
                #pred_unk = np.absolute(ent1-entr)
                pred_unk = ent1
                #pred_unk1 = 
                ind_unk1 = np.where(label_t.data.cpu().numpy() == 20)[0]
                ind_k1 = np.where(label_t.data.cpu().numpy() != 20)[0]

                ind_unk = np.where(pred_unk > thr)[0]
                lo(ent1[ind_k1], ent1[ind_unk1])
            else:
                out_open = Cs[1](feat)
                out_open = F.softmax(out_open.view(out_t.size(0), 2, -1),1)
                tmp_range = torch.range(0, out_t.size(0)-1).long().cuda()
            ...
            if open :
                ...
                if batch_idx == 0:
                    ...
                    pred_open = np.r_[pred_open, pred_unk]
                    pred_ent = np.r_[pred_ent, ent1]
...
def select_threshold(pred_all, conf_thr, label_all,
    ...
    if thr is not None:
        ...
        return accuracy_score(label_all, pred_class), \
               ...
               accuracy_score(label_all, pred_class)
    ran = np.linspace(0.0, 2.0, num=40)
